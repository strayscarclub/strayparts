...
- Stay conservative.
- Preserve relevance.
- Do not broaden unless it clearly helps.

Rules:
- Return at most 2 alternate queries.
- For very specific searches, return no alternate queries unless they are extremely close naming variations.
- For category-style searches, you may return a small number of highly relevant enthusiast alternatives.
- Do not generate overly broad generic terms.
- Do not generate terms that drift outside the likely intended part.
- Preserve chassis codes, engine codes, model names, part names, and enthusiast shorthand.
- Japanese or translated equivalents are allowed only if they would realistically help retrieval.
...
- "race seats" -> a small number of strong enthusiast alternatives is okay
- "bucket seats" -> a small number of strong enthusiast alternatives is okay

Bad behaviour:
- Do not turn a specific search into a generic category search.
- Do not generate weakly related terms just to fill space.
- Do not generate more than 2 alternates.
`;
